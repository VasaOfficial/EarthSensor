  ...
  const fetchAqiData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `https://earth-sensor.vercel.app/api/geolocation?lat=${lat}&lng=${lng}`,
      )
      if (res.ok) {
          ...
          <>
            ...
            {cityName !== 'here' ? (
              <div className="flex flex-col gap-2">
                ...
                <p className="opacity-70 text-sm mt-3">
                  Last updated{' '}
                  {aqiData.data.time.iso &&
                    formatDistanceToNow(new Date(aqiData.data.time.iso), { addSuffix: true })}
                </p>
              </div>
            ) : (
              ''
            ...
            <div className="flex flex-col md:flex-row gap-6 mt-12">
              ...
              <div className="flex flex-col gap-6 w-full h-fit md:w-1/2">
                  ...
                  <div className="flex flex-col gap-4 p-4 w-full bg-neutral-200 rounded-md">
                    ...
                    {Object.keys(aqiData.data.forecast.daily)
                      ...
                      .map((keyName, index) => {
                        ...
                        const currentIaqi = aqiData.data.iaqi[keyName]
                ...
                <div
                  id="tooltip"
                  className="z-50 fixed hidden w-64 text-sm p-2 rounded-md top-0 left-0 backdrop-blur border-solid border-black shadow-lg pointer-events-none text-black bg-emerald-400 leading-4"
                ></div>
              </div>
            </div>
            {/* --- WEATHER ---*/}
            <Weather weatherData={weatherData} />
            {!refreshMap && <AqiMap coordinates={coordinates} />}
            <RandomFacts
              bg={
                aqiData.data.aqi >= 301
                  ? 'bg-[#A159FF]/50'
